...
func (a *SbomAPIService) GetSbomComponentsExecute(r ApiGetSbomComponentsRequest) (*http.Response, error) {
	...
	if r.vulnerabilityThreatLevels != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "vulnerabilityThreatLevels", s.Index(i).Interface(), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "vulnerabilityThreatLevels", t, "multi")
		}
	}
	if r.dependencyTypes != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "dependencyTypes", s.Index(i).Interface(), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "dependencyTypes", t, "multi")
		}
	}
	if r.sortBy != nil {
...
// Target specification of the sbom. Allowed values [cyclonedx1.5|spdx2.3]. default &#x3D; cyclonedx1.5
...
func (a *SbomAPIService) GetSbomVersionExecute(r ApiGetSbomVersionRequest) (*http.Response, error) {
	...
	if r.specification != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "specification", r.specification, "")
	} else {
		var defaultValue string = "cyclonedx1.5"
