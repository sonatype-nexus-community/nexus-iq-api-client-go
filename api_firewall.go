...
type ApiAddBulkWaiversRequest struct {
	ctx context.Context
	ApiService *FirewallAPIService
	ownerType string
	ownerId string
	apiBulkWaiversDTO *ApiBulkWaiversDTO
}

// Bulk waiver request containing: - **violationIds** (required, 1-1000 items): List of repository policy violation IDs to waive. Duplicate IDs are automatically deduplicated. Supports both quarantine (FAIL) and non-quarantine (WARN) violations. Already-waived violations are skipped without error. - **apiWaiverOptionsDTO** (required): Waiver options including:   - **comment** (required, non-blank): Reason for waiving these violations   - **matcherStrategy** (required): EXACT_COMPONENT or ALL_VERSIONS   - **expiryTime** (optional): Must be in the future if provided   - **waiverReasonId** (optional): Reference to a pre-defined waiver reason   - **expireWhenRemediationAvailable** (optional): If true, matcherStrategy must be EXACT_COMPONENT
func (r ApiAddBulkWaiversRequest) ApiBulkWaiversDTO(apiBulkWaiversDTO ApiBulkWaiversDTO) ApiAddBulkWaiversRequest {
	r.apiBulkWaiversDTO = &apiBulkWaiversDTO
	return r
}

func (r ApiAddBulkWaiversRequest) Execute() (*http.Response, error) {
	return r.ApiService.AddBulkWaiversExecute(r)
}

/*
AddBulkWaivers Method for AddBulkWaivers

Bulk-waive up to 1000 repository policy violations for Firewall. Supports both quarantine violations (FAIL action) and non-quarantine violations (WARN action). Quarantined components are released on the next monitoring or reevaluation pass once all remaining FAIL violations have been waived.

This operation executes within a single database transaction - if any violation fails validation, the entire request is rolled back (no partial waivers are created).

**Performance**: Requests with 100 violations typically complete in <5 seconds, requests with 1000 violations complete in <10 seconds.

**Permissions required**: Waive Policy Violations

**Security**: All violations must belong to the authenticated owner's tenant hierarchy. Cross-tenant access attempts will be rejected.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param ownerType Owner type scope for the waiver. Must be one of: organization, repository, repository_manager, repository_container. Use 'organization' with ownerId 'ROOT_ORGANIZATION_ID' to apply waiver across all repositories.
 @param ownerId Owner ID corresponding to the owner type. All violations must belong to this owner's tenant hierarchy.
 @return ApiAddBulkWaiversRequest
*/
func (a *FirewallAPIService) AddBulkWaivers(ctx context.Context, ownerType string, ownerId string) ApiAddBulkWaiversRequest {
	return ApiAddBulkWaiversRequest{
		ApiService: a,
		ctx: ctx,
		ownerType: ownerType,
...
func (a *FirewallAPIService) AddBulkWaiversExecute(r ApiAddBulkWaiversRequest) (*http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FirewallAPIService.AddBulkWaivers")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/firewall/repositories/{ownerType}/{ownerId}/waivers/bulk"
	...
	if r.apiBulkWaiversDTO == nil {
		return nil, reportError("apiBulkWaiversDTO is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.apiBulkWaiversDTO
...
func (a *FirewallAPIService) AddProprietaryComponentNamesExecute(r ApiAddProprietaryComponentNamesRequest) (*http.Response, error) {
	...
	localVarPath := localBasePath + "/api/v2/firewall/namespace_confusion/{format}"
...
// Enter values for the new repository.
func (r ApiAddRepositoryRequest) ApiRepositoryDTO(apiRepositoryDTO ApiRepositoryDTO) ApiAddRepositoryRequest {
	r.apiRepositoryDTO = &apiRepositoryDTO
	return r
}

func (r ApiAddRepositoryRequest) Execute() (*ApiRepositoryDTO, *http.Response, error) {
...
Use this method to add a new repository to a virtual repository manager.
...
*/
func (a *FirewallAPIService) AddRepository(ctx context.Context, repositoryManagerId string) ApiAddRepositoryRequest {
	return ApiAddRepositoryRequest{
		ApiService: a,
		ctx: ctx,
...
func (a *FirewallAPIService) AddRepositoryExecute(r ApiAddRepositoryRequest) (*ApiRepositoryDTO, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *ApiRepositoryDTO
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FirewallAPIService.AddRepository")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/firewall/repositoryManagers/{repositoryManagerId}/repositories"
	...
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.apiRepositoryDTO == nil {
		return localVarReturnValue, nil, reportError("apiRepositoryDTO is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.apiRepositoryDTO
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
...
type ApiAddVirtualRepositoryManagerRequest struct {
	ctx context.Context
	ApiService *FirewallAPIService
	apiRepositoryManagerDTO *ApiRepositoryManagerDTO
}
...
// Enter values for the new repository manager.
func (r ApiAddVirtualRepositoryManagerRequest) ApiRepositoryManagerDTO(apiRepositoryManagerDTO ApiRepositoryManagerDTO) ApiAddVirtualRepositoryManagerRequest {
	r.apiRepositoryManagerDTO = &apiRepositoryManagerDTO
	return r
}

func (r ApiAddVirtualRepositoryManagerRequest) Execute() (*ApiRepositoryManagerDTO, *http.Response, error) {
	return r.ApiService.AddVirtualRepositoryManagerExecute(r)
}

/*
AddVirtualRepositoryManager Method for AddVirtualRepositoryManager
...
Permissions required: Edit IQ Elements
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiAddVirtualRepositoryManagerRequest
*/
func (a *FirewallAPIService) AddVirtualRepositoryManager(ctx context.Context) ApiAddVirtualRepositoryManagerRequest {
	return ApiAddVirtualRepositoryManagerRequest{
		ApiService: a,
		ctx: ctx,
	}
...
// Execute executes the request
//  @return ApiRepositoryManagerDTO
func (a *FirewallAPIService) AddVirtualRepositoryManagerExecute(r ApiAddVirtualRepositoryManagerRequest) (*ApiRepositoryManagerDTO, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FirewallAPIService.AddVirtualRepositoryManager")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/firewall/virtualManagers"
...
func (a *FirewallAPIService) AddWaiverExecute(r ApiAddWaiverRequest) (*http.Response, error) {
	...
	localVarPath := localBasePath + "/api/v2/firewall/container-image/{containerImageId}/policyWaiver"
...
type ApiGetCascadeStatusRequest struct {
	ctx context.Context
	ApiService *FirewallAPIService
	requestId string
}

func (r ApiGetCascadeStatusRequest) Execute() (*CascadeStatusResponseDTO, *http.Response, error) {
	return r.ApiService.GetCascadeStatusExecute(r)
}

/*
GetCascadeStatus Method for GetCascadeStatus
...
Get the status of a cascade re-evaluation request.<p>Returns the current progress of a cascade re-evaluation operation including the list of components that have been evaluated and those still pending. The overall status will be 'pending' if any components are still being processed, or 'completed' if all components have been evaluated.<p>The response includes:<ul><li><b>status:</b> Overall status (PENDING, IN_PROGRESS, COMPLETED, NO_COMPONENTS_FOUND, FAILED)</li><li><b>referenceComponentHash:</b> The component hash that was re-evaluated</li><li><b>pending:</b> Components still being processed (PENDING status)</li><li><b>evaluated:</b> Components successfully re-evaluated (COMPLETED or NO_COMPONENTS_FOUND status)</li><li><b>failed:</b> Components that could not be re-evaluated (FAILED status)</li></ul><p>Permissions Required: Evaluate Components at Repository Managers level
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param requestId The cascade request ID to check status for
 @return ApiGetCascadeStatusRequest
*/
func (a *FirewallAPIService) GetCascadeStatus(ctx context.Context, requestId string) ApiGetCascadeStatusRequest {
	return ApiGetCascadeStatusRequest{
		ApiService: a,
		ctx: ctx,
		requestId: requestId,
...
// Execute executes the request
//  @return CascadeStatusResponseDTO
func (a *FirewallAPIService) GetCascadeStatusExecute(r ApiGetCascadeStatusRequest) (*CascadeStatusResponseDTO, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *CascadeStatusResponseDTO
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FirewallAPIService.GetCascadeStatus")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/firewall/repositories/cascade-reevaluate/status/{requestId}"
...
func (a *FirewallAPIService) GetContainerImagesInQuarantineExecute(r ApiGetContainerImagesInQuarantineRequest) (*ContainerImageInQuarantineDataResult, *http.Response, error) {
	...
	localVarPath := localBasePath + "/api/v2/firewall/container-image/policyViolations/quarantined"
...
func (r ApiGetPolicyWaiverRequest) Execute() (*http.Response, error) {
...
Retrieve a single Firewall policy waiver by ID. <p>This endpoint is for Firewall Dashboard use only. Authorization is based on Firewall repository-level access rather than owner-level READ permission, allowing scoped users to view waivers visible in their Firewall Dashboard list.<p>For scoped users, the waiver owner must be reachable from their permitted proxy repositories: a permitted repository itself, an organization or repository-manager ancestor of one, or a container-image application linked to a permitted repository via its shadow organization. Waivers at root-organization or repository-manager scope are accessible to any authenticated user with access to at least one proxy repository.<p>Permissions Required: READ permission on at least one proxy repository.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param ownerType Owner type (application, organization, repository, repository_manager, or repository_container)
 @param ownerId Owner ID
 @param policyWaiverId Policy waiver ID
 @return ApiGetPolicyWaiverRequest
*/
func (a *FirewallAPIService) GetPolicyWaiver(ctx context.Context, ownerType string, ownerId string, policyWaiverId string) ApiGetPolicyWaiverRequest {
	return ApiGetPolicyWaiverRequest{
		ApiService: a,
		ctx: ctx,
		ownerType: ownerType,
...
// Execute executes the request
func (a *FirewallAPIService) GetPolicyWaiverExecute(r ApiGetPolicyWaiverRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FirewallAPIService.GetPolicyWaiver")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/firewall/policyWaivers/{ownerType}/{ownerId}/{policyWaiverId}"
	localVarPath = strings.Replace(localVarPath, "{"+"ownerType"+"}", url.PathEscape(parameterValueToString(r.ownerType, "ownerType")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"ownerId"+"}", url.PathEscape(parameterValueToString(r.ownerId, "ownerId")), -1)
...
func (a *FirewallAPIService) GetWaiversExecute(r ApiGetWaiversRequest) (*PolicyContainerWaiverDataResult, *http.Response, error) {
	...
	localVarPath := localBasePath + "/api/v2/firewall/container-image/policyWaiver"
...
type ApiInitiateCascadeReevaluationRequest struct {
	ctx context.Context
	ApiService *FirewallAPIService
	componentHash string
}

func (r ApiInitiateCascadeReevaluationRequest) Execute() (*CascadeReevaluateTicketDTO, *http.Response, error) {
	return r.ApiService.InitiateCascadeReevaluationExecute(r)
}

/*
InitiateCascadeReevaluation Method for InitiateCascadeReevaluation
...
Initiate cascade re-evaluation for a component across repository hierarchies.<p>This operation asynchronously re-evaluates the specified component across all repositories where the component exists.<p>The system will automatically discover all eligible repositories based on component presence.<p>Permissions Required: Evaluate Components at Repository Managers level
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param componentHash The component hash to re-evaluate across all repositories
 @return ApiInitiateCascadeReevaluationRequest
*/
func (a *FirewallAPIService) InitiateCascadeReevaluation(ctx context.Context, componentHash string) ApiInitiateCascadeReevaluationRequest {
	return ApiInitiateCascadeReevaluationRequest{
		ApiService: a,
		ctx: ctx,
...
// Execute executes the request
//  @return CascadeReevaluateTicketDTO
func (a *FirewallAPIService) InitiateCascadeReevaluationExecute(r ApiInitiateCascadeReevaluationRequest) (*CascadeReevaluateTicketDTO, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *CascadeReevaluateTicketDTO
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FirewallAPIService.InitiateCascadeReevaluation")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/firewall/repositories/cascade-reevaluate/componentHash/{componentHash}"
...
type ApiRenewWaiversRequest struct {
	ctx context.Context
	ApiService *FirewallAPIService
	renewWaiversRequestDTO *RenewWaiversRequestDTO
}

// Renewal request containing:\\n- **waiverIds** (required): List of waiver IDs to renew\\n- **newExpiryTime** (optional): New expiry time as ISO 8601 timestamp. Null means the waiver never expires.\\n- **comment** (optional): Comment explaining the renewal\\n- **reasonId** (optional): ID of a pre-defined waiver reason
func (r ApiRenewWaiversRequest) RenewWaiversRequestDTO(renewWaiversRequestDTO RenewWaiversRequestDTO) ApiRenewWaiversRequest {
	r.renewWaiversRequestDTO = &renewWaiversRequestDTO
	return r
}

func (r ApiRenewWaiversRequest) Execute() (*http.Response, error) {
	return r.ApiService.RenewWaiversExecute(r)
}

/*
RenewWaivers Method for RenewWaivers

Renews Firewall policy waivers with a new expiry date. Records the previous expiry time for audit and tracking purposes.\n\n**Supports both single and bulk renewals**: Pass one or more waiver IDs in the request body.\n\n**Permissions required**: Waive Policy Violations\n\n**Security**: Permission is checked against the waiver's owner. The authenticated user must have WAIVE_POLICY_VIOLATIONS on the owner scope.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiRenewWaiversRequest
*/
func (a *FirewallAPIService) RenewWaivers(ctx context.Context) ApiRenewWaiversRequest {
	return ApiRenewWaiversRequest{
		ApiService: a,
		ctx: ctx,
	}
...
// Execute executes the request
func (a *FirewallAPIService) RenewWaiversExecute(r ApiRenewWaiversRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FirewallAPIService.RenewWaivers")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/firewall/waivers/renew"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.renewWaiversRequestDTO == nil {
		return nil, reportError("renewWaiversRequestDTO is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.renewWaiversRequestDTO
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return nil, err
	}
...
type ApiRequestWaiverRequest struct {
	ctx context.Context
	ApiService *FirewallAPIService
	containerImageId string
	apiContainerImageWaiverRequestDTO *ApiContainerImageWaiverRequestDTO
}

// Fields: comment, noteToReviewer, expiryTime, waiverReasonId
func (r ApiRequestWaiverRequest) ApiContainerImageWaiverRequestDTO(apiContainerImageWaiverRequestDTO ApiContainerImageWaiverRequestDTO) ApiRequestWaiverRequest {
	r.apiContainerImageWaiverRequestDTO = &apiContainerImageWaiverRequestDTO
	return r
}

func (r ApiRequestWaiverRequest) Execute() (*http.Response, error) {
	return r.ApiService.RequestWaiverExecute(r)
}

/*
RequestWaiver Method for RequestWaiver

Use this method to request a waiver for all policy violations of a container image. 

Permissions required: authenticated user (waiver request workflow must be enabled)

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param containerImageId The container image id.
 @return ApiRequestWaiverRequest
*/
func (a *FirewallAPIService) RequestWaiver(ctx context.Context, containerImageId string) ApiRequestWaiverRequest {
	return ApiRequestWaiverRequest{
		ApiService: a,
		ctx: ctx,
		containerImageId: containerImageId,
...
// Execute executes the request
func (a *FirewallAPIService) RequestWaiverExecute(r ApiRequestWaiverRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FirewallAPIService.RequestWaiver")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/firewall/container-image/{containerImageId}/policyWaiverRequest"
	localVarPath = strings.Replace(localVarPath, "{"+"containerImageId"+"}", url.PathEscape(parameterValueToString(r.containerImageId, "containerImageId")), -1)
	...
	// body params
	localVarPostBody = r.apiContainerImageWaiverRequestDTO
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return nil, err
	}
...
type ApiVerifyConnectionAndGetApplicationsRequest struct {
	ctx context.Context
	ApiService *FirewallAPIService
}

func (r ApiVerifyConnectionAndGetApplicationsRequest) Execute() (*ApplicationSummaryList, *http.Response, error) {
	return r.ApiService.VerifyConnectionAndGetApplicationsExecute(r)
}

/*
VerifyConnectionAndGetApplications Method for VerifyConnectionAndGetApplications
...
Permissions required: Evaluate Individual Components
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiVerifyConnectionAndGetApplicationsRequest
*/
func (a *FirewallAPIService) VerifyConnectionAndGetApplications(ctx context.Context) ApiVerifyConnectionAndGetApplicationsRequest {
	return ApiVerifyConnectionAndGetApplicationsRequest{
		ApiService: a,
		ctx: ctx,
	}
...
func (a *FirewallAPIService) VerifyConnectionAndGetApplicationsExecute(r ApiVerifyConnectionAndGetApplicationsRequest) (*ApplicationSummaryList, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *ApplicationSummaryList
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FirewallAPIService.VerifyConnectionAndGetApplications")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/firewall/connection/verify"
