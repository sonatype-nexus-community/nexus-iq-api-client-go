...
Get evaluation metadata for a specific scan. Requires READ permission on the application.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param applicationPublicId Enter the applicationPublicId for the evaluated application.
 @param scanId Enter the scanId (reportId) of the application report.
 @return ApiGetMetadataRequest
*/
func (a *ApplicationReportDataAPIService) GetMetadata(ctx context.Context, applicationPublicId string, scanId string) ApiGetMetadataRequest {
	return ApiGetMetadataRequest{
		ApiService: a,
		ctx: ctx,
		applicationPublicId: applicationPublicId,
...
// Execute executes the request
func (a *ApplicationReportDataAPIService) GetMetadataExecute(r ApiGetMetadataRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ApplicationReportDataAPIService.GetMetadata")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/applications/{applicationPublicId}/reports/{scanId}/metadata"
	localVarPath = strings.Replace(localVarPath, "{"+"applicationPublicId"+"}", url.PathEscape(parameterValueToString(r.applicationPublicId, "applicationPublicId")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"scanId"+"}", url.PathEscape(parameterValueToString(r.scanId, "scanId")), -1)
...
// Page number (1-indexed). Must be provided together with &#39;pageSize&#39;. When omitted, all components are returned.
func (r ApiGetPolicyViolations1Request) Page(page int32) ApiGetPolicyViolations1Request {
	r.page = &page
	return r
}

// Number of components per page (1-500). Must be provided together with &#39;page&#39;. When omitted, all components are returned.
func (r ApiGetPolicyViolations1Request) PageSize(pageSize int32) ApiGetPolicyViolations1Request {
	r.pageSize = &pageSize
	return r
}
...
type ApiGetRawDataRequest struct {
	...
	applicationPublicId string
	scanId string
	includeCustomSecurityVulnerabilityData *bool
}

// Set to true to include security vulnerability custom data (remediation, cweId, cvssVector, cvssSeverity) for each securityIssue. Defaults to false.
func (r ApiGetRawDataRequest) IncludeCustomSecurityVulnerabilityData(includeCustomSecurityVulnerabilityData bool) ApiGetRawDataRequest {
	r.includeCustomSecurityVulnerabilityData = &includeCustomSecurityVulnerabilityData
	return r
}
...
Use this method to retrieve the 'raw' data generated as a result of an application evaluation. 'raw' data includes: the components identified in the application, and the licenses and vulnerabilities associated with the identified components. Optionally set `includeCustomSecurityVulnerabilityData=true` to include any configured security vulnerability custom data (remediation, cweId, cvssVector, cvssSeverity) for each securityIssue. NOT to be confused with `SecurityVulnerabilityOverride` (hash-based violation-state override).
...
func (a *ApplicationReportDataAPIService) GetRawDataExecute(r ApiGetRawDataRequest) (*ApiReportRawDataDTOV2, *http.Response, error) {
	...
	if r.includeCustomSecurityVulnerabilityData != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "includeCustomSecurityVulnerabilityData", r.includeCustomSecurityVulnerabilityData, "form", "")
	} else {
		var defaultValue bool = false
		r.includeCustomSecurityVulnerabilityData = &defaultValue
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}
