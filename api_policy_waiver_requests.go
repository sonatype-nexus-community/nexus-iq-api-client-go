...
type ApiGetPolicyWaiverRequestsRequest struct {
	ctx context.Context
	ApiService *PolicyWaiverRequestsAPIService
	ownerType string
	ownerId string
...
// Optional filter for repository format. Use &#39;docker&#39; to return only container repository waiver requests, or &#39;component&#39; to return only non-container repository waiver requests. Omit to return all.
func (r ApiGetPolicyWaiverRequestsRequest) RepositoryFormat(repositoryFormat string) ApiGetPolicyWaiverRequestsRequest {
	r.repositoryFormat = &repositoryFormat
	return r
}

func (r ApiGetPolicyWaiverRequestsRequest) Execute() ([]ApiPolicyWaiverRequestDTO, *http.Response, error) {
	return r.ApiService.GetPolicyWaiverRequestsExecute(r)
}

/*
GetPolicyWaiverRequests Method for GetPolicyWaiverRequests

Use this method to list all policy waiver requests for the given owner.

Permissions required: View IQ Elements

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param ownerType The owner type. Possible values are application, organization, repository, repository_manager, repository_container.
 @param ownerId The id for the ownerType.
 @return ApiGetPolicyWaiverRequestsRequest
*/
func (a *PolicyWaiverRequestsAPIService) GetPolicyWaiverRequests(ctx context.Context, ownerType string, ownerId string) ApiGetPolicyWaiverRequestsRequest {
	return ApiGetPolicyWaiverRequestsRequest{
		ApiService: a,
		ctx: ctx,
		ownerType: ownerType,
...
// Execute executes the request
//  @return []ApiPolicyWaiverRequestDTO
func (a *PolicyWaiverRequestsAPIService) GetPolicyWaiverRequestsExecute(r ApiGetPolicyWaiverRequestsRequest) ([]ApiPolicyWaiverRequestDTO, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "PolicyWaiverRequestsAPIService.GetPolicyWaiverRequests")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/policyWaiverRequests/{ownerType}/{ownerId}"
	localVarPath = strings.Replace(localVarPath, "{"+"ownerType"+"}", url.PathEscape(parameterValueToString(r.ownerType, "ownerType")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"ownerId"+"}", url.PathEscape(parameterValueToString(r.ownerId, "ownerId")), -1)
	...
	if r.repositoryFormat != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "repositoryFormat", r.repositoryFormat, "form", "")
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}
